...
def lets_get_these_wvfns(type, switch_speed):
    if type == 'min':
        ...
        plt.plot(grid, np.mean(GSW[0], axis=0)/np.linalg.norm(np.mean(GSW[0], axis=0)), color='orange', label='Average Ground State Wave function')
        plt.plot(grid, GSW[0, 2, :], color='green', label='Low Frequency CH stretch')
        plt.plot(grid, GSW[0, 4, :], color='red', label='High Frequency CH stretch')
        # plt.plot(grid, GSW[0, 0, :], color='blue', label='Ground State Wave Function 1')
        # plt.plot(grid, GSW[0, 1, :], color='purple', label='Ground State Wave Function 2')
        # plt.plot(grid, GSW[0, 2, :], color='green', label='Ground State Wave Function 3')
        # plt.plot(grid, GSW[0, 3, :], color='teal', label='Ground State Wave Function 4')
        # plt.plot(grid, GSW[0, 4, :], color='red', label='Ground State Wave Function 5')
