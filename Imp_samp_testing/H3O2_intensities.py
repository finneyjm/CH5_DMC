...
def full_intensity_calcs(filename):
    ...
    def get_da_psi(coords, excite):
        ...
        if excite == 'sp':
            ...
            psi[:, 1] = interp(dists[:, -1], dists[:, -2], excite_no_der)
        elif excite == 'a':
            psi[:, 0] = (mw_h / np.pi) ** (1. / 4.) * np.exp(-(1. / 2. * mw_h * dists[:, 0] ** 2)) * \
                        (2 * mw_h) ** (1 / 2) * dists[:, 0]
    ...
    frac = np.zeros(xh_combine_weights.shape)
    frac2 = np.zeros(xh_combine_weights.shape)
