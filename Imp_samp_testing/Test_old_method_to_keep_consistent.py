...
def drift(psi):
    dpsidx = (psi[:, 2] - psi[:, 0])/(2.*dx)/psi[:, 1]
    return 2.*dpsidx


def E_loc(Psi, sigmaCH, dtau):
    rch = ch_dist(Psi.coords)
    d2psidx2 = ((Psi.psit[:, 0] - 2.*Psi.psit[:, 1] + Psi.psit[:, 2])/dx**2)/Psi.psit[:, 1]
    # weights = np.array([-1/12, 4/3, -5/2, 4/3, -1/12], dtype=float)
    # d2psidx2 = np.tensordot(weights/dx**2, Psi.psit, axes=[0, 1])/Psi.psit[:, 1]
...
psi, E_time = tm.time_me(E_loc, psi, sigmaCH, dtau)
