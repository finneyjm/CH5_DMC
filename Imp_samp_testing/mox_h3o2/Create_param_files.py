...
for i in range(10):
    for type_of_sim in sim:
        with open(f'brute_force_h3o2_dmc_{type_of_sim}_{mod}_{i + 1}_param.py', 'w') as myfile:
            ...
            if type_of_sim == 'asym_left' or type_of_sim == 'asym_right':
                myfile.write('        20000, 20000, 250, 500, 500, "a", test_structure, [0, 2.5721982410729867], [0, 0]\n')
            elif type_of_sim == 'XH_left' or type_of_sim == 'XH_right':
                myfile.write('        20000, 20000, 250, 500, 500, "sp", test_structure, [0, 2.5721982410729867], [0, 0]\n')
            elif type_of_sim == 'ground':
                myfile.write('        20000, 20000, 250, 500, 500, None, test_structure, [0, 2.5721982410729867], [0, 0]\n')
            myfile.write('    )\n')
            myfile.write(f'np.savez(f"{type_of_sim}_excite_state_{mod}_h3o2_{i+1}", coords=coords, weights=weights, time=time, Eref=Eref_array,\n')
