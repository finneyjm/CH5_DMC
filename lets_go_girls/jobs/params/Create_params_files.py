...
for i in range(5):
    for x in range(len(walkers)):
        with open(f'params_Non_imp_sampled_CD_{walkers[x]}_walkers_test_{i+1}.py', 'w') as myfile:
            myfile.write('import numpy as np\n\n')
            myfile.write('atoms = ["C", "D", "D", "D", "D", "D"]\n')
            ...
            myfile.write(f'    "output": "Non_imp_sampled_CD_{walkers[x]}_Walkers_Test_{i+1}",\n')
            ...
            myfile.write('}\n\n')
            myfile.write(f'output_dir = "Non_imp_sampled_CD"\n')
            myfile.close()
