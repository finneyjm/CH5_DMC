...
for j in range(6):
    for i in range(5):
        for x in range(len(walkers)):
            with open(f'params_HH_to_rCHrCD_{j}H_GSW2_{walkers[x]}_walkers_test_{i+1}.py', 'w') as myfile:
                ...
                myfile.write(f'    "output": "HH_to_rCHrCD_{j}H_GSW2_{walkers[x]}_Walkers_Test_{i+1}",\n')
                ...
                myfile.write('}\n\n')
                myfile.write(f'output_dir = "HH_to_rCHrCD_{j}H_GSW2"\n')
                myfile.close()
