...
for i in range(5):
    for x in range(len(walkers)):
        with open(f'params_Non_imp_sampled_4H_{walkers[x]}_walkers_test_{i+1}.py', 'w') as myfile:
            myfile.write('import numpy as np\n\n')
            myfile.write('atoms = ["C", "H", "H", "H", "H", "D"]\n')
            # myfile.write(f'wvfn = np.load("params/min_wvfns/Min_CD_average.npy")\n')
            ...
            myfile.write(f'    "output": "Non_imp_sampled_4H_{walkers[x]}_Walkers_Test_{i+1}",\n')
            ...
            myfile.write('}\n\n')
            myfile.write(f'output_dir = "Non_imp_sampled_4H"\n')
            myfile.close()
