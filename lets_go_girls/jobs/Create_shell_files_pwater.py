...
#     for i in range(5):
    for a, b in zip(size, s):
        with open(f'DMC_ptetramer_non_imp_samp_ts_{ts}_{a}.sh', 'w') as myfile:
            myfile.write('#!/bin/bash\n\n')
            myfile.write('## Job Name\n\n')
            myfile.write(f'#SBATCH --job-name=ptetramer_non_imp_samp_ts_{ts}_{b}\n\n')
            ...
            myfile.write(f'python ptetramer_non_imp_samp_ts_{ts}_subproc_{a}.py\n')
            ...
            myfile.write(f'echo "ptetramer_non_imp_samp_ts_{ts}_{b} took: $DIFF"')
