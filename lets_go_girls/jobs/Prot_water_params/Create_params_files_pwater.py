...
for i in range(5):
    for x in range(len(walkers)):
        with open(f'params_Non_imp_sampled_pdimer_{walkers[x]}_walkers_test_{i+1}.py', 'w') as myfile:
            ...
            # myfile.write('trial_wvfn = {"reg_oh": free_oh_wvfn, "ang": None, "hbond": None}\n')
            ...
            myfile.write(f'    "output": "Non_imp_sampled_pdimer_{walkers[x]}_Walkers_Test_{i+1}",\n')
            ...
            myfile.write('}\n\n')
            myfile.write(f'output_dir = "Non_imp_sampled_pdimer"\n')
            myfile.close()
