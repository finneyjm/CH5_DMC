...
plt.scatter(np.std(hh[0], axis=1)/ang2bohr, zmat[0, :, 1]/ang2bohr, label='Minimum geometry')
plt.scatter(np.std(hh_cs[0], axis=1)/ang2bohr, zmat_cs[0, :, 1]/ang2bohr, label=r'$C_s$ saddle point')
plt.scatter(np.std(hh_c2v[0], axis=1)/ang2bohr, zmat_c2v[0, :, 1]/ang2bohr, label=r'$C_{2v}$ saddle point')
plt.plot(new_hh/ang2bohr, new_rch/ang2bohr)
plt.plot(g/ang2bohr, exp_fit(g, *fitted_params)/ang2bohr, label='Exponential fit')
# plt.plot(g, quad_fit(g, *fitted_params2), label='quadratic fit')
plt.legend(fontsize=14)
plt.xlabel(r'$\sigma_{HH}$ ($\AA$)', fontsize=22)
plt.ylabel(r'r$_{CH}$ ($\AA$)', fontsize=22)
