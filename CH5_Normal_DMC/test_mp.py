...
plt.scatter(np.std(hh[0], axis=1), zmat[0, :, 1], label='Minimum geometry')
plt.scatter(np.std(hh_cs[0], axis=1), zmat_cs[0, :, 1], label=r'$C_s$ saddle point')
plt.scatter(np.std(hh_c2v[0], axis=1), zmat_c2v[0, :, 1], label=r'$C_{2v}$ saddle point')
plt.plot(new_hh, new_rch)
plt.plot(g, exp_fit(g, *fitted_params), label='Exponential fit')
# plt.plot(g, quad_fit(g, *fitted_params2), label='quadratic fit')
plt.legend(fontsize=14)
plt.xlabel(r'$\sigma_{HH}$', fontsize=16)
plt.ylabel('rCH (Bohr)', fontsize=16)
