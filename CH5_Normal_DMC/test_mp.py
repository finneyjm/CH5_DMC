...
plt.scatter(np.std(hh[0], axis=1), zmat[0, :, 1]/ang2bohr, label='Minimum geometry')
plt.scatter(np.std(hh_cs[0], axis=1), zmat_cs[0, :, 1]/ang2bohr, label=r'$C_s$ saddle point')
plt.scatter(np.std(hh_c2v[0], axis=1), zmat_c2v[0, :, 1]/ang2bohr, label=r'$C_{2v}$ saddle point')
plt.plot(new_hh, new_rch/ang2bohr)
plt.plot(g, exp_fit(g, *fitted_params)/ang2bohr, label='Exponential fit')
# plt.plot(g, quad_fit(g, *fitted_params2), label='quadratic fit')
plt.legend(fontsize=14)
plt.xlabel(r'$\sigma_{HH}$', fontsize=16)
plt.ylabel(r'r$_{CH}$ ($\AA$)', fontsize=16)
plt.tick_params(axis='both', which='major', labelsize=12)
plt.xlim(0, 0.4)
plt.ylim(2.04/ang2bohr, 2.34/ang2bohr)
plt.tight_layout
