...
for j in np.arange(0, 5):
    Energy = np.load(f'Trial_wvfn_testing/results/ptetramer_full_imp_samp_deuterated/' +
                     f'ptetramer_full_imp_samp_deuterated_{non_imp_samp_walkers}_' +
                     f'Walkers_Test_{j + 1}.npz')['Eref'] * har2wave
    a = -0.122146858971399 * har2wave
    energies_imp[j] = np.mean(Energy[5000:]) - a

print('non imp 30000 mean = ' + str(np.mean(energies_imp)))
print('non imp 30000 std = ' + str(np.std(energies_imp)))
