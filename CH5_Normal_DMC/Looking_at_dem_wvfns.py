...
def regular_wvfns(walkers, alpha):
    bings = 50
    fig, axes = plt.subplots(1, 5, figsize=(25, 8))
    amp_all = np.zeros((2, 5, 5, bings))
    for i in range(5):
        coords = np.load(f'Trial_wvfn_testing/alpha_{alpha}/coords/' +
                         f'Imp_samp_DMC_CH5_randomly_sampled_coords_alpha_{alpha}_{walkers}_walkers_{i+1}.npy')
        zmat = CoordinateSet(coords, system=CartesianCoordinates3D).convert(ZMatrixCoordinates, ordering=order).coords
        weights = np.load(f'Trial_wvfn_testing/alpha_{alpha}/weights/' +
                          f'Imp_samp_DMC_CH5_randomly_sampled_weights_alpha_{alpha}_{walkers}_walkers_{i+1}.npy')

        for CH in range(5):
            amp, xx = np.histogram(zmat[:, CH, 1] / ang2bohr, weights=weights[1, :], bins=bings, range=(0.6, 1.8),
                                   density=True)
            bins = (xx[1:] + xx[:-1]) / 2.
        ...
        # coords = np.load(f'Non_imp_sampled/DMC_CH5_randomly_sampled_coords_{walkers}_walkers_{i+1}.npy')
        # zmat = CoordinateSet(coords, system=CartesianCoordinates3D).convert(ZMatrixCoordinates, ordering=order).coords
        # weights = np.load(f'Non_imp_sampled/DMC_CH5_randomly_sampled_weights_{walkers}_walkers_{i+1}.npy')
        # for CH in range(5):
        #     amp, xx = np.histogram(zmat[:, CH, 1]/ang2bohr, weights=weights[1, :], bins=bings, range=(0.6, 1.8), density=True)
        #     bins = (xx[1:] + xx[:-1])/2.
        #     amp_all[1, i, CH, :] += amp
    ...
    for CH in range(5):
        ...
        axes[CH].plot(bins, np.mean(amp_all[0, :, CH, :], axis=0), color='red', label=r'$f$(x) w/ imp samp')
        axes[CH].set_title(f'CH stretch {CH + 1}')
        axes[CH].set_xlabel('rCH (Angstrom)')
        axes[CH].set_ylabel('Probability Density')
