...
class Walkers(object):
    ...
    def __init__(self, walkers):
        ...
        self.coords = np.array([coords_inital]*walkers)*1.01
        # np.random.seed(24)
...
def run(propagation, test_number):
    ...
    for i in range(int(time_total)):
        ...
        new_psi = Weighting(Vref, Psi, DW)
        print(f'weights = {new_psi.weights}')

        Vref = V_ref_calc(new_psi)
        print(f'V_ref= {Vref}')
        if i == 0:
            print(new_psi.coords)
        #     print(new_psi.weights)
        #     print(Vref)
        Eref = np.append(Eref, Vref)
