...
def lets_get_some_energies(non_imp_samp_walkers, imp_samp_walkers, trials_ni, trials_i, broad):
    ...
    for j in range(trials_ni):
        for i in range(N_n):
            Energy = np.load(f'Trial_wvfn_testing/results/Non_imp_sampled_4H/Non_imp_sampled_4H_{non_imp_samp_walkers[i]}_' +
                             f'Walkers_Test_{j+1}.npz')['Eref']*har2wave
            energies_non[i, j] += np.mean(Energy[5000:])
    for j in range(trials_i):
        for i in range(N_i):
            Energy = np.load(f'Trial_wvfn_testing/results/DD_to_rCD_4H/' +
                             f'DD_to_rCD_4H_{imp_samp_walkers[i]}_' +
                             f'Walkers_Test_{j+1}.npz')['Eref']*har2wave
    ...
    axes[0].plot(non_imp_samp_walkers, [10300]*len(non_imp_samp_walkers), color='purple')
    axes[1].errorbar(imp_samp_walkers, avg_imp, yerr=std_imp, color='blue', label='Imp Sampling')
    axes[1].plot(imp_samp_walkers, [10300] * len(imp_samp_walkers), color='purple')
    ...
    axes[1].legend()
    plt.tight_layout()
    fig.savefig(f'Convergence_plots/Energy_convergence_CH5_DD_to_rCD_4H.png')
    # plt.close(fig)
