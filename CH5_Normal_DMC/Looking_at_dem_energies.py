...
def lets_get_some_energies(non_imp_samp_walkers, imp_samp_walkers, trials_ni, trials_i, broad):
    ...
    for j in range(trials_ni):
        for i in range(N_n):
            Energy = np.load(f'Trial_wvfn_testing/results/Non_imp_sampled/Non_imp_sampled_{non_imp_samp_walkers[i]}_' +
    ...
    axes[1].legend()
    plt.tight_layout()
    fig.savefig(f'Convergence_plots/Energy_convergence_CH5_HH_to_rCH.png')
    # plt.close(fig)
...
# for i in bro:
lets_get_some_energies(walkers3, walkers3, 5, 5, 4)
plt.show()